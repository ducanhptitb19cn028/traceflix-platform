...
GET  /api/live/cluster/info           DEPLOYED cluster: TF_LIVE, Chaos Mesh truth, training set
GET  /api/live/cluster/stream         SSE: detection on real telemetry, labelled by live Chaos CRs
GET  /api/melt/info                   MELT pillar catalogue + which engine feeds the page
GET  /api/melt/windows                recent raw MELT windows (backfill for the MELT page)
...
# The recorded campaign the write-up's live result comes from, and the directory
# holding it. Both are the live-replay defaults and both are protected below: a
# replay of any OTHER labels file must be given its own out directory, because
# build_live_windows resumes from live_windows_cache.jsonl and returns every
# window that cache holds -- so two campaigns sharing a directory are scored as
# one, silently, and the run still looks clean.
DEFAULT_LIVE_LABELS = "data/labels_live.csv"
DEFAULT_LIVE_OUT = "data/results_live"
...
EXPERIMENTS = {
    ...
    "live-replay": {
        ...
        "args": lambda p: ["--labels", p["labels"], "--out", p["out"]],
        "params": ["labels", "out"],
        "out": DEFAULT_LIVE_OUT,
        ...
        "note": "Needs a reachable Prometheus still holding the campaign's retention "
                ...
                "collectors take no timestamp. To score a campaign you injected "
                "yourself, point Labels at the file faults/inject.py wrote and give it "
                "its OWN out directory — one directory per campaign, never two.",
    },
    # ---- exports -----------------------------------------------------------
    "excel": {
        "group": "Exports",
...
def _rel_to_aiops(raw: str, field: str) -> str:
    """A caller-supplied path, confined to the aiops tree.

    These two land in a subprocess argv, so there is no shell to inject into;
    the guard is against a path that escapes the tree or is simply mistyped --
    live_replay treats an absent labels file as a traceback and an odd out
    directory as a fresh one, neither of which reads as "you typed it wrong".
    """
    root = AIOPS.resolve()
    p = (root / raw).resolve()
    if root not in p.parents and p != root:
        raise HTTPException(400, f"{field} must stay under aiops/: {raw}")
    return p.relative_to(root).as_posix()


def _live_paths(labels: str, out: str) -> tuple[str, str]:
    """Validate a live-replay campaign pair and keep the two apart.

    The reported campaign's directory is reserved for the reported campaign. Any
    other labels file writing into it would be scored against the union of both
    (see DEFAULT_LIVE_OUT above), so refuse the pairing rather than produce a
    result that looks fine and is not.
    """
    labels = _rel_to_aiops(labels, "labels")
    out = _rel_to_aiops(out, "out")
    if not (AIOPS / labels).is_file():
        raise HTTPException(400, f"no such labels file: {labels}")
    if out == DEFAULT_LIVE_OUT and labels != DEFAULT_LIVE_LABELS:
        raise HTTPException(
            400, f"{DEFAULT_LIVE_OUT}/ is reserved for {DEFAULT_LIVE_LABELS} — "
                 f"give this campaign its own out directory, e.g. "
                 f"{DEFAULT_LIVE_OUT}_{Path(labels).stem}")
    return labels, out
...
@app.get("/api/live/cluster/info")
def live_cluster_info():
    """Status of the DEPLOYED-cluster detector: is TF_LIVE on, is Chaos Mesh
    readable for ground truth, and how many live windows the models were fitted
    on. All three decide whether anything this page reports means anything."""
    from streaming.cluster_detect import cluster_info
    return cluster_info()


@app.get("/api/live/cluster/stream")
def live_cluster_stream(request: Request):
    return _engine_sse(request, "cluster")


@app.get("/api/melt/info")
def melt_page_info(kind: str = "cluster"):
    """Pillar catalogue for the MELT page, plus which engine's windows it will
    show — the deployed mesh (`cluster`) or the generator (`ml`/`llm`)."""
    if kind not in ("ml", "llm", "cluster"):
        raise HTTPException(404, f"unknown engine {kind}")
    from streaming.live_detect import melt_info
    return melt_info(kind)


@app.get("/api/melt/windows")
def melt_windows(kind: str = "cluster", service: str | None = None, limit: int = 1350):
    """The recent raw MELT windows the engine has collected, oldest first.

    Backfill for the MELT page: the SSE snapshot carries only the window just
    scored, so without this a viewer would watch an empty page fill one service
    at a time and would lose it all on reload. Attaching does NOT start scoring —
    the engine is already running — but it does start it if nobody has yet, the
    same as every other live endpoint."""
    if kind not in ("ml", "llm", "cluster"):
        raise HTTPException(404, f"unknown engine {kind}")
    from streaming.live_detect import get_engine
    rows = get_engine(kind).melt_history()
    if service:
        rows = [r for r in rows if r.get("service") == service]
    rows = rows[-max(1, min(limit, 5000)):]
    return {"kind": kind, "count": len(rows), "windows": rows}
...
def live_control(kind: str, rate: float | None = None, paused: bool | None = None,
    ...
    there is one live detector, not one per browser tab. On the cluster engine
    `rate` is seconds between collected instants, not windows per second: its
    throughput is set by how fast Prometheus answers, not by a chosen sleep."""
    if kind not in ("ml", "llm", "cluster"):
        raise HTTPException(404, f"unknown engine {kind}")
    if kind == "cluster" and config is not None:
        raise HTTPException(
            400, "the cluster engine is fixed at C1: its models are fitted on "
                 "replay caches whose C2-C4 pillars do not belong to their label")
    eng = get_engine(kind)
    if rate is not None:
        eng.set_rate(rate)
...
async def offline_run(request: Request, key: str, episodes: int = 200,
                      ...
                      labels: str = DEFAULT_LIVE_LABELS, out: str = DEFAULT_LIVE_OUT):
    if key not in EXPERIMENTS:
        raise HTTPException(400, f"unknown experiment {key}")
    spec = EXPERIMENTS[key]
    if "labels" in spec["params"]:
        labels, out = _live_paths(labels, out)
    p = {"episodes": episodes, "configs": configs, "seeds": seeds,
         "labels": labels, "out": out}
    args = spec["args"](p)
    argv = [sys.executable, "-u", "-m", spec["module"], *args]
    # An experiment that takes `out` writes where the caller said, not where the
    # registry's default says, or the completion check would look in the wrong
    # directory and report a successful run as having produced nothing.
    out_rel = p["out"] if "out" in spec["params"] else spec["out"]
    out_dir = AIOPS / out_rel
