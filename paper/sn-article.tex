...
We approach this question with a controlled study that fixes the detection pipeline and varies, orthogonally, the telemetry it is given and the way it learns, on a single production-representative testbed under controlled fault injection and controlled drift. We begin under \emph{stationary} conditions, where the field's prevailing intuitions can be checked against a clean ceiling. The most basic of those intuitions is that more telemetry is better, so the first question we ask is:

\rqbox{RQ1.}{Under stationary conditions, does the completeness of observability data---metrics-only, ${+}$logs, ${+}$traces, full MELT---improve the accuracy of machine-learning anomaly detection, and which pillar contributes the largest marginal gain?}

\noindent RQ1 fixes the attainable detection ceiling and the marginal return on each pillar of instrumentation. A given signal set can be exploited well or badly, however, and a fair ceiling demands the right model; we therefore ask:

\rqbox{RQ2.}{Which model family best exploits rich, multimodal observability features, and what does that choice imply for the model that must run in the online setting?}

\noindent Detection is only half of operations: an alert becomes actionable only once the responsible service is named. Because traces alone carry the cross-service causal structure that metrics and logs lack, we isolate their specific contribution to localisation:

\rqbox{RQ3.}{What is the marginal contribution of distributed traces, specifically, to root-cause localisation, holding all other signals constant?}

\noindent RQ1--RQ3 all inhabit a \emph{stationary} world---a model trained once and evaluated on the same distribution. Production is not stationary: deployments, autoscaling and shifting load continuously reshape the telemetry, so a boundary fitted to yesterday's normal silently decays. This is the crux of the article, and it turns the governing question from \emph{how much} telemetry into \emph{whether the detector keeps learning}:

\rqbox{RQ4.}{When the telemetry stream is non-stationary, how do the static-batch, periodically-retrained and online-adaptive learning paradigms compare on detection quality \emph{and} operational cost, and how does this comparison interact with telemetry completeness?}

\noindent RQ1--RQ3 are deliberately posed in the established terms of the observability literature, so that our stationary results can be checked against prior expectations; RQ4 then re-uses the \emph{identical} pipeline and feature sets on a drifting stream, so that any reversal of conclusions is attributable to non-stationarity rather than to a change of model, data, or task.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item[\emph{To RQ1:}] detection quality rises \emph{monotonically} with telemetry richness (F1 $0.906 \rightarrow 0.994$), and distributed traces are the single most valuable pillar---larger than logs or events.
    \item[\emph{To RQ2:}] ensemble trees are the strongest family on multimodal tabular telemetry, which is also why a lightweight \emph{normalised linear} model---not a tree---is the right detector for the online setting.
    \item[\emph{To RQ3:}] adding traces lifts root-cause top-1 localisation from $0.91$ to a perfect $1.0$ on our topology, isolating tracing as the decisive localisation signal.
    \item[\emph{To RQ4:}] under drift the result \emph{inverts}---a frozen batch model collapses to F1 $\approx 0.49$--$0.51$ \emph{regardless} of telemetry, periodic retraining only partially recovers, and a lightweight online-adaptive detector dominates at every configuration and in $10$ of $12$ regime cells, reaching F1 $\approx 0.98$ and even beating an all-regimes oracle, while being $17$--$32\times$ better on tail latency and $\sim 200$--$550\times$ smaller.
  \end{itemize}
...
\subsection{RQ1: Does richer telemetry raise the detection ceiling?}\label{sec:res-rq1}
...
\subsection{RQ2: Which model family best exploits multimodal telemetry?}\label{sec:res-rq2}
...
\subsection{RQ3: What does distributed tracing add to localisation?}\label{sec:res-rq3}
...
\subsection{RQ4: Does the learning paradigm matter under drift?}\label{sec:res-rq4}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{fig_rq4_headline.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{fig_rq4_cost.pdf}
