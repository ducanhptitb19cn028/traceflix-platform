...
\begin{itemize}
  ...
  \begin{itemize}
    \item[\emph{To RQ1:}] under non-stationarity, telemetry completeness still improves detection: for a detector that keeps current, drifted-stream F1 rises \emph{monotonically} from $0.82$ to $0.98$ across the configurations (mirroring $0.906 \rightarrow 0.994$ on a held-out reference), with distributed traces the single most valuable pillar---and decisively so under drift, where the trace increment ($+0.147$ F1) dwarfs that of logs or events.
    \item[\emph{To RQ2:}] adding traces lifts root-cause top-1 localisation from $0.91$ to a perfect $1.0$ on our topology, isolating tracing as the decisive localisation signal.
    \item[\emph{To RQ3:}] under drift the result \emph{inverts}---a frozen batch model collapses to F1 $\approx 0.49$--$0.51$ \emph{regardless} of telemetry, periodic retraining only partially recovers, and a lightweight online-adaptive detector dominates at every configuration and in $10$ of $12$ regime cells, reaching F1 $\approx 0.98$ and even beating an all-regimes oracle, while being $17$--$32\times$ better on tail latency and $\sim 200$--$550\times$ smaller.
...
\subsection{A taxonomy of the design space}\label{sec:fnd-taxonomy}

Figure~\ref{fig:taxonomy} organises cloud-native anomaly detection along six dimensions. The first four classify a detector's \emph{design}; the last two classify the \emph{conditions} under which it is evaluated.
...
\begin{forest}
  for tree={
    ...
    inner sep=2.6pt, l sep=13pt, s sep=2.4pt,
  },
  forked edges,
  ...
  [Cloud-native\\anomaly\\detection
    [D1: Telemetry input
      [metrics,hl][+logs,hl][+traces,hl][full MELT,hl]]
    [D2: Detection model
      [statistical / threshold][classical ML][deep learning][graph / topology]]
    [D3: Learning paradigm
      [static-batch,hl][periodic retrain,hl][online / continual,hl]]
    [D4: Task
      [detection,hl][localisation / RCA,hl][prediction]]
    [D5: Drift posture
      [stationary][drift-aware,hl]]
    [D6: Evaluation setting
      [benchmark / synthetic][controlled-drift testbed,hl][production]]]
\end{forest}
\caption{A taxonomy of cloud-native anomaly detection along six dimensions---four of \emph{design} (D1--D4) and two of \emph{evaluation} (D5--D6). Shaded leaves mark the region the present study occupies: it holds the detection model fixed (D2) as a control while sweeping telemetry completeness (D1) and the learning paradigm (D3) \emph{together}, evaluating detection and localisation under controlled drift (D5--D6). The bulk of prior work fixes D3 to static-batch and D5 to stationary.}\label{fig:taxonomy}
\end{figure}

\emph{Telemetry input} (D1) is the signal a detector consumes, ranging from a single modality---most often metrics \cite{lomio2022,mart2020}---through progressively richer combinations to full MELT, the multimodal end at which the strongest recent results cluster \cite{han2024,wang2025multimodal}. \emph{Detection model} (D2) is the function it fits: statistical and threshold rules; classical machine learning such as tree ensembles, isolation forests and one-class methods \cite{lomio2022}; deep learning such as autoencoders, recurrent and transformer architectures \cite{dkmak2025,wang2025mtssurvey}; and graph models that encode the service topology and its span timings \cite{chen2023tracegra,han2024}. \emph{Learning paradigm} (D3) is how, and how often, the model is updated: a \emph{static} model is trained once and frozen; a \emph{periodically-retrained} model is refitted on a sliding buffer at fixed intervals; and an \emph{online} model updates incrementally on every window \cite{wette2024omlad,li2023autoencoder}. This dimension is orthogonal to the choice of model---any model family can in principle be deployed in any paradigm---and it is the second axis the study manipulates. \emph{Task} (D4) is detection, localisation or prediction, as above.

The two evaluation dimensions are easy to overlook yet decisive. \emph{Drift posture} (D5) records whether a detector is tested on a held-fixed distribution (stationary, single-snapshot) or on a moving one (drift-aware) \cite{lukats2025drift}. \emph{Evaluation setting} (D6) records the provenance of the telemetry: a public benchmark or synthetic generator \cite{bakhtin2025lo2,pham2025rcaeval}; an operational production system \cite{islam2021,hrusto2026}; or---the middle path this study takes---a production-representative testbed under controlled fault injection and controlled drift.

Read against this map, the field is unevenly populated. Dimensions D1, D2 and D4 are densely and actively worked: the literature has produced an abundance of modalities, models and tasks, and continues to advance each. Dimensions D3 and D5, by contrast, are nearly collapsed. The overwhelming majority of cloud-native detectors are static-batch models evaluated on a stationary slice of a single distribution \cite{lomio2022,dkmak2025,han2024}, even though adaptability to a moving distribution is a named---and, by recent accounting, systematically unmet---requirement \cite{raeiszadeh2026}. The shaded leaves in Figure~\ref{fig:taxonomy} mark the region this paper occupies. Rather than advance any single cell, it holds the detection model fixed (D2, a deliberate control) and sweeps the telemetry-input axis (D1) and the learning-paradigm axis (D3) \emph{together}, evaluating both detection and localisation (D4) under drift (D5) on a controlled-drift testbed (D6). It is the crossing of D1 with D3---not progress within either alone---that the study contributes, and the formal apparatus for that crossing is the subject of the next section.

%%===========================================================%%
\section{Problem Formulation}\label{sec:problem}
%%===========================================================%%

We now make the task and the two design axes precise: the \emph{telemetry-completeness} axis (dimension~D1) and the \emph{learning-paradigm} axis (dimension~D3) of the taxonomy in Figure~\ref{fig:taxonomy}, whose crossing was identified there as the under-explored region this study occupies.
...
\end{description}
The stream is processed prequentially over $320$ episodes and $11{,}520$ windows, of which $2{,}880$ constitute the $R_0$ warm-up and $8{,}640$ the drifted future ($R_1$--$R_3$) on which RQ3 is scored.
...
\midrule
C1 & Metrics-Only            & 10 & 0.9064 & 0.8174 & --- \\
C2 & Metrics${+}$Logs        & 14 & 0.9332 & 0.8347 & $+0.017$ \\
C3 & ${+}$Traces             & 18 & 0.9882 & \textbf{0.9817} & $+0.147$ \\
C4 & Full MELT               & 23 & 0.9937 & \textbf{0.9834} & $+0.002$ \\
\botrule
\end{tabular}
\end{table}

Detection quality rises \emph{monotonically} with telemetry richness in both settings. On the held-out reference, F1 climbs from $0.906$ (metrics-only) to $0.994$ (full MELT); on the operative drifted stream, the continual-learning detector improves from $0.817$ to $0.983$. In both, the decisive increment is the addition of distributed traces (C2$\rightarrow$C3), and that increment is markedly larger under drift---$+0.147$ F1 on the drifted stream against $+0.055$ on the reference---because traces resolve precisely the cross-service signal whose statistics shift most under the load and scale changes that drive the drift. Adding logs (C1$\rightarrow$C2) yields a smaller gain in both settings, and adding events and history (C3$\rightarrow$C4) leaves detection essentially unchanged.
...
Table~\ref{tab:rq2} reports root-cause localisation with traces excluded (C2) versus included (C3). The introduction of traces raises top-1 accuracy from $0.9068$ to a perfect $1.0000$, eliminating the localisation errors the metric-and-log approach commits.

\begin{table}[t]
\caption{RQ2 --- root-cause localisation, traces excluded vs.\ included. Top-1 is the discriminating metric on a three-service topology.}\label{tab:rq2}
\begin{tabular}{@{}lcc@{}}
\toprule
...
RQ1 and RQ2 measured a controlled, \emph{held-out baseline}---detection accuracy and root-cause localisation for a model trained once on a single distribution. We now re-use the identical pipeline and feature sets on the drifted future stream ($8{,}640$ post-warm-up windows across $R_1$--$R_3$) and compare the three realisable learning policies, with the all-regimes oracle as a ceiling.

\subsubsection{Headline: F1 on the drifted stream}\label{sec:res-rq3-head}

Table~\ref{tab:rq3-head} and Fig.~\ref{fig:rq3head} report F1 over the future stream. The result inverts the baseline picture. The frozen batch model collapses to F1 $\approx 0.49$--$0.51$ \emph{regardless} of how much telemetry it is given: going from metrics-only to full MELT moves its F1 by barely two points ($0.4894 \rightarrow 0.5112$). Scheduled retraining recovers substantially (up to $\sim 0.89$) but always trails. The online-adaptive model \emph{wins at every configuration}, reaching $0.9817$ under C3 and $0.9834$ under C4---and, notably, exceeding the all-regimes oracle ($0.9387$ at C4; Table~\ref{tab:rq3-regime} note), because the oracle still fits a single static boundary across all regimes whereas the online model tracks each regime as it arrives.
...
\begin{table}[t]
\caption{RQ3 per-regime F1 by configuration. Best learner per cell in bold. (s = static, p = periodic, o = online.) The all-regimes oracle attains 0.812 (C1), 0.834 (C2), 0.940 (C3), 0.939 (C4); the online model exceeds it at C3--C4.}\label{tab:rq3-regime}
\begin{tabular}{@{}llccc@{}}
\toprule
Config & Regime & s & p & o \\
\midrule
...
\begin{table}[t]
\caption{RQ4 --- detection by model family on full MELT (C4). Ensemble trees lead.}\label{tab:rq4}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
This paper asked whether, for a non-stationary observability stream, the learning paradigm matters more than telemetry richness, and how the two interact. By crossing four nested telemetry configurations with three learning policies on a single production-representative, fault-injected, drift-controlled testbed, we reached a two-part answer. On the held-out baseline, completeness is decisive: detection F1 rises monotonically from $0.906$ to $0.994$, traces are the single most valuable increment, ensemble trees are the strongest family, and traces lift root-cause top-1 localisation to a perfect $1.0$. But under drift this ceiling does not survive a frozen model: the static detector collapses to F1 $\approx 0.49$--$0.51$ regardless of telemetry, periodic retraining only partially and expensively recovers, and a lightweight online-adaptive detector dominates every configuration and the large majority of regimes---reaching $\approx 0.98$ under full MELT, beating an all-regimes oracle---while being $17$--$32\times$ better on tail latency, $\sim 200$--$550\times$ smaller, and free of any data-retention requirement.
